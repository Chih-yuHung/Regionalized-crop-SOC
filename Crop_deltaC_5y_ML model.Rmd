...
# MSE: 0.001 | RMSE: 0.025 | R²: 0.935 | MAPE: 78.48 % | sMAPE: 33.23 %
# 
# ----- Running model for TEXT = 2 -----
# Unique values in deltaSOC: 8378 
# MSE: 0.001 | RMSE: 0.036 | R²: 0.888 | MAPE: 139.9 % | sMAPE: 42.35 %
# 
# ----- Running model for TEXT = 3 -----
# Unique values in deltaSOC: 4022 
# MSE: 0.002 | RMSE: 0.047 | R²: 0.859 | MAPE: 157.72 % | sMAPE: 50.77 %
```


## XGBosst without spliting soil texture


```{r XGboost without spliting soil texture, eval=FALSE}


# Prepare matrices (XGBoost needs numeric matrices)
train_x <- train_data %>%
  select(all_of(selected_vars)) %>%
  mutate_if(is.factor, as.numeric) %>%
  as.matrix()
...
# MAPE
mape <- if (any(test_y == 0)) NA else mean(abs((pred - test_y) / test_y)) * 100

# sMAPE
smape <- mean(2 * abs(pred - test_y) / (abs(pred) + abs(test_y))) * 100

# Output
cat("\n===== XGBoost (Full Dataset) Evaluation =====\n")
cat("MSE:", round(mse, 3),
    "| RMSE:", round(rmse, 3),
    "| R²:", round(r2, 3),
    "| MAPE:", round(mape, 2), "%",
...
#MSE: 0.001 | RMSE: 0.034 | R²: 0.900 | MAPE: 87.13 % | sMAPE: 39.38 %
