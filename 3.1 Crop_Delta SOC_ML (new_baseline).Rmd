---
title: "3.1 Crop_Delta SOC_Random Forest (new baseline)"
...
file_paths  <- list.files("Input", pattern = "^SiteData_\\d+y_\\(new baseline\\)\\.rds$", full.names = TRUE)
...
#for (i in 5) {
  ...
  # Save model
  save(rf_model, file = file.path("Output", paste0(model_name, "(new baseline).RData")))
  ...
  ggsave(filename = file.path("Output", paste0(model_name, "_plot_(new baseline).png")), plot = p, width = 6, height = 5)
...
# Save summaries
write.csv(perf_table, "Output/Model_Performance_Moving average_(new baseline).csv", row.names = FALSE)
write.csv(imp_wide, "Output/Variable_Importance_Comparison_Moving average_(new baseline).csv", row.names = FALSE)

# Show performance
print(perf_table)

#perf_table <- read.csv("Output/Model_Performance_Moving average.csv")
