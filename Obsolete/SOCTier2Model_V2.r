...
# Dated 4 May 2018
# Script to call to the IPCC Tier 2 SOC stock calculations 

library(tidyverse)

IPCCTier2SOMmodel <- function(SiteData, wth, params, init.active,init.slow,init.passive)
{
  #  AUTHOR: 
  ...
  for(year in years){
    ...
    tempSOM <- data.frame("site" = site, "year" = year+1, "dfac" = dfac, 
  ...
  SOMstock<-SOMstocks%>%select(year, active, slow, passive, TotSOC)
  SOMstock[nrow(SOMstock),]=c(min(years), init.active, init.slow, init.passive, sum(init.active, init.passive, init.slow))
  SOMstock%>%filter(year<=max(years))%>%arrange(year)->SOMstock
  return(SOMstock)
}
